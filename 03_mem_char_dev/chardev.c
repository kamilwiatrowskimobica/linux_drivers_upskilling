...
static int woab_device_alloc_buffer(size_t buffer_length)
{
	...
	woab_device->data = (char*)kmalloc(buffer_length * sizeof(char), GFP_KERNEL);
	if (!woab_device->data) {
...
static int __init woabchar_init(void)
{
	...
	ret = alloc_chrdev_region(&dev, woabchar_minor, 
				woabchar_nr_devices, "woabchar");
...
static ssize_t woab_dev_write(struct file *filep, const char *buffer, size_t len,
			 loff_t *offset)
{
	int uncopied, ret;
