		   ...
		   loff_t *f_pos)
{
	...
	if (count > device_max_size) {
		printk(KERN_WARNING
		       "[LEO] hello: trying to read more than possible. Aborting read\n");
		retval = -EFBIG;
		goto out;
	}
		    ...
		    loff_t *f_pos)
{
	...
	if (copy_from_user((void *)hello_devices->p_data, buf, count)) {
		printk(KERN_WARNING
		       "[LEO] hello: can't use copy_from_user. \n");
		retval = -EPERM;
		goto out;
	}
	return retval;
...
static void hello_setup_cdev(struct hello_dev *dev)
{
	...
	if (err)
		printk(KERN_WARNING "[LEO] Error %d adding hello cdev_add",
		       err);
...
static int hello_init(void)
{
	...
	if (hello_major) {
		printk(KERN_INFO
		       "[LEO] static allocation of major number (%d)\n",
		       hello_major);
