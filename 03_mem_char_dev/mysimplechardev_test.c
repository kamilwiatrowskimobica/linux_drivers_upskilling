...
int writeReadTest(int *fds, char stringsToSend[][BUFFER_LENGTH])
{
	int ret = 0;
...
int writeByteByByteReadTest(int *fds, char stringsToSend[][BUFFER_LENGTH])
{
	int ret = 0;
	char receive[BUFFER_LENGTH];
	for (int i = 0; i < DEVICES_COUNT; i++) {
...
int writeSeekReadTest(int *fds, char stringsToSend[][BUFFER_LENGTH])
{
	int ret = 0;
	char receive[BUFFER_LENGTH];
...
int writeSeekWriteSeekReadTest(int *fds, char stringsToSend[][BUFFER_LENGTH])
{
	int ret = 0;
	char receive[BUFFER_LENGTH];
	char tmp[BUFFER_LENGTH];
...
void printStatus(char *msg, int retCode)
{
	if (retCode)
		printf("%s FAILED!\n", msg);
	else
