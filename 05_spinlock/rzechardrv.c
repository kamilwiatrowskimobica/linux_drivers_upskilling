...
static int __init rzechardrv_init(void){
	...
	dev_t devt = 0;
        ...
        for(i = 0; i < rzechar_nr_devs; i++){  
                ...
                rwlock_init(&my_devs[i].my_rwlock);
...
ssize_t dev_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
        ...
        read_lock_irqsave(&dev->my_rwlock, flags);
        ...
        *f_pos = *f_pos + count;
        ...
        read_unlock_irqrestore(&dev->my_rwlock, flags);
...
ssize_t dev_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
        ...
        write_lock_irqsave(&dev->my_rwlock, flags);
        ...
        *f_pos = *f_pos + count;
        write_unlock_irqrestore(&dev->my_rwlock, flags);
