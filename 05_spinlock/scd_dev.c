 ...
 * @date   21 Jule 2023
 * @version 1.0
 * @brief   Character driver with proc for chapter 5 of Linux Device Driver book (sync with spinlock - see readme)
 */
...
MODULE_DESCRIPTION("Character driver with proc for chapter 5 of Linux Device Driver book (sync with spinlock - see readme)");
MODULE_VERSION("1.0");
...
static int __init scd_dev_init(void)
{
    ...
    for (int i = 0; i < SCD_DEV_COUNT; i++)
    {
        ...
        scd_devs[i]->pos = 0;
        scd_devs[i]->id = i;
        spin_lock_init(&scd_devs[i]->spinlock);
...
static ssize_t scd_dev_read(struct file *filep, char *buffer, size_t len, loff_t *offset)
{
    ...
    spin_lock(&mdev->spinlock);

    if (mdev->pos > mdev->buffer_size)
    {
        spin_unlock(&mdev->spinlock);
        return 0;
    }
