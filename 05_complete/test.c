...
int test_read_race_condition()
{
    ...
    for (i = 0; i < 100; i++)
    {
        ...
        fd = open(get_device(SCD_0), O_RDWR);

        if ((ret = read(fd, buff, BUFFER_LENGTH)) < 0)
        {
            printf("Failed to read the data from the device(%d).\n", ret);
            break;
        }
...
int main()
{
    ...
    //run_test("read from empty device", test_read_empty); // we can't read before write with write completion check
