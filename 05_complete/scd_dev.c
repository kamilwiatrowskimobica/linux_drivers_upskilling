 ...
 * @date   21 Jule 2023
 * @version 1.0
 * @brief   Character driver with proc for chapter 5 of Linux Device Driver book (sync with mutex and wait for write completion)
 */
...
MODULE_DESCRIPTION("Character driver with proc for chapter 5 of Linux Device Driver book (sync with mutex and wait for write completion)");
MODULE_VERSION("1.0");
...
static void __exit scd_dev_exit(void)
{
    ...
    printk(KERN_INFO "scd_dev: Write COMPLETE EXIT \n");
    if (!completion_done(&write_done))
    {
        complete(&write_done);
    }
...
static ssize_t scd_dev_read(struct file *filep, char *buffer, size_t len, loff_t *offset)
{
    ...
    if ((ret = wait_for_completion_interruptible(&write_done)))
    {
        printk(KERN_ALERT "scd_dev: read: wait_for_completion error: %d. \n", ret);
        return -ERESTARTSYS;
    }

    if (mutex_lock_interruptible(&mdev->sem))
    {
        printk(KERN_ALERT "scd_dev: read: mutex lock. \n");
        return -ERESTARTSYS;
    }
    ...
    *offset = mdev->pos;
    ...
    {
        printk(KERN_ALERT "scd_dev: mdev->pos > mdev->buffer_size. \n");
    ...
    if (len > mdev->pos)
    {
        printk(KERN_ALERT "scd_dev: len > mdev->pos. \n");
        len = mdev->pos;
    }
...
static ssize_t scd_dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset)
{
    ...
    *offset = mdev->pos;
    ...
    complete(&write_done);
    mutex_unlock(&mdev->sem);
    printk(KERN_INFO "scd_dev: Write COMPLETE \n");
