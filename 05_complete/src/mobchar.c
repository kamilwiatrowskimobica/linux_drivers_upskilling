...
static int __init mobchar_init(void)
{
	...
	init_completion(&(mobchar_devices->comp));
...
{
	int error_count = 0;
	long wait_for_completion_ret;

	wait_for_completion_ret = wait_for_completion_interruptible_timeout(
		&(mobchar_devices->comp), COMPLETION_TIMEOUT);
	if (0 == wait_for_completion_ret) {
		DEBUG_PRINT("Timeout\n");
		return 0;
	} else if (-ERESTARTSYS == wait_for_completion_ret) {
		DEBUG_PRINT("Interruped\n");
		return 0;
	} else {
		DEBUG_PRINT("Time left = %ld\n", wait_for_completion_ret);
	}
	DEBUG_PRINT("awoken %i (%s)\n", current->pid, current->comm);
				 ...
				 size_t len, loff_t *offset)
{
	...
	if (error_count == 0) {
		...
		logk(KERN_INFO, "Process %d named %s completed writting\n",
		     current->pid, current->comm);
		complete(&(mobchar_devices->comp));
		return len;
	} else {
