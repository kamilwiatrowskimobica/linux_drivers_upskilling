...
#include <linux/slab.h> // kmalloc(), kmem_cache
...
#define DEVICE_CACHE_NAME "MOBI_CACHE"
...
void mobdev_cache_ctor(void *cache)
{
		pr_info_ratelimited("MOBChar: mobdev cache Constructor\n");
}
...
static void mobdev_clean_cdev(struct mob_dev *mdev)
{
	...
	if (mob_cache) {
		if (mdev->msg)
			kmem_cache_free(mob_cache, mdev->msg);
	}
...
static int mobdev_init_cdev(struct mob_dev *mdev, size_t index)
{
	...
	mdev->msg = kmem_cache_alloc(mob_cache, GFP_KERNEL);
	if (IS_ERR(mdev->msg)) {
...
static int __init mobdev_init(void)
{
	...
	mob_cache = kmem_cache_create_usercopy(DEVICE_CACHE_NAME, MOB_MESSAGE_LEN, 0, SLAB_HWCACHE_ALIGN, 0, MOB_MESSAGE_LEN, mobdev_cache_ctor);
	if (!mob_cache) {
		exit_handler(CLEANUP_U_REGION | CLEANUP_CLASS_DESTROY);
		return -ENOMEM;
	}
