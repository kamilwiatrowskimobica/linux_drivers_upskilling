...
struct kmem_cache *mk_mob_cache;
...
static void mk_mob_cleanup_module(void)
{
	...
	if (mk_mob_devices) {
		for (i = 0; i < nr_devs; i++) {
			if (mk_mob_devices[i].data) {
				kmem_cache_free(mk_mob_cache, mk_mob_devices[i].data);
			}
	...
	if (mk_mob_cache)
		kmem_cache_destroy(mk_mob_cache);
...
static void mk_mob_setup_cdev(struct mk_mob_dev *dev, size_t index)
{
	...
	dev->data = kmem_cache_alloc(mk_mob_cache, GFP_KERNEL);
	dev->size = 0;
...
static int __init mk_mob_init(void)
{
	...
	/* Create the cache for our device data */
	mk_mob_cache = kmem_cache_create_usercopy("mk_mob_cache", MK_DEV_MEMALLOC_MAX_SIZE, 0, SLAB_HWCACHE_ALIGN, 0, MK_DEV_MEMALLOC_MAX_SIZE, NULL);
	if (!mk_mob_cache) {
