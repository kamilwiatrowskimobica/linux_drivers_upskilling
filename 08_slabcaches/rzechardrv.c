...
struct kmem_cache *rze_dev_cache;
...
static int __init rzechardrv_init(void){
	...
	dev_t devt = 0;
        ...
        rze_dev_cache = kmem_cache_create_usercopy(DEVICE_NAME, DEVICE_MAX_SIZE, 0,
					   SLAB_HWCACHE_ALIGN, 0, DEVICE_MAX_SIZE, NULL);
	if (!rze_dev_cache) {
		class_destroy(rzecharClass);
                unregister_chrdev(rzechar_major, DEVICE_NAME);
                printk(KERN_ALERT "Failed to alloc slab cache for the device\n");
		return -ENOMEM;
	}
        ...
        for(i = 0; i < rzechar_nr_devs; i++){  
                ...
                my_devs[i].p_cache = kmem_cache_alloc(rze_dev_cache, GFP_KERNEL);
	        if (!my_devs[i].p_cache) {
                        result = -ENOMEM;
		        printk(KERN_WARNING "Failed to alocate cache buffer\n");
		        return result;
	        }
...
static void __exit rzechardrv_exit(void){
        ...
        for(i = 0; i < rzechar_nr_devs; i++){  
		        ...
		        kfree(my_devs[i].p_data);
                        kmem_cache_free(rze_dev_cache, my_devs[i].p_cache);
	...
	if ((my_devs) != 0) {
        ...
        if (rze_dev_cache)
		kmem_cache_destroy(rze_dev_cache);
...
ssize_t dev_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	//         return -EFAULT;
	// }
        if (copy_to_user(buf, dev->p_cache + *f_pos, count)) {
                printk(KERN_WARNING "rzechar: can't copy to user. \n");
	        return -EFAULT;
	}
...
ssize_t dev_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	//         return -EFAULT;
        // }
        if (copy_from_user(dev->p_cache + *f_pos, buf, count)){
                printk(KERN_WARNING "rzechar: can't copy from  user. \n");
	        return -EFAULT;
        }
