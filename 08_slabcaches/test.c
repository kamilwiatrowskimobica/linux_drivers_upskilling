...
void slab_tests(void)
{
	...
	memset(write, pattern_1_value, sizeof(write));
	memset(read, pattern_2_value, sizeof(read));

	for (i = 0; i < size; i++) {
		printf("\tTesting %s device for slab\n", ptr[i]);
		fd = open(ptr[i], O_RDWR);
		if (fd < 0) {
			printf("Cannot open device file...\n");
			return;
		}
		printf("\tWrite %ld bytes to device %s\n", sizeof(write), ptr[i]);
		ioctl(fd, WOAB_DEVICE_WRITE_CACHE, write);
		printf("\tRead %ld bytes from device %s\n", sizeof(read), ptr[i]);
		ioctl(fd, WOAB_DEVICE_READ_CACHE, read);
		if(0 == memcmp(read, write, sizeof(read)))
			printf("\tTest passed, pattern match\n");
		else
