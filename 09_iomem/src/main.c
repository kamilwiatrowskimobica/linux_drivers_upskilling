...
void* writing_thread(void *arg)
{
	...
	for (int i=0; i<data->it_count; i++) {
		...
		for (int j=0; j<data->it_count; j++) {
			...
			if (ret < 0) {
				perror("writing_thread: Failed to seek the device.");
				break;
			}
		}
	}
...
void* reading_thread(void *arg)
{
	...
	for (int i=0; i<data->it_count; i++) {
		...
		for (int j=0; j<data->it_count; j++) {
			...
			ret = lseek(fd, 0, SEEK_SET);
			if (ret < 0) {
				perror("reading_thread: Failed to seek the device.");
				break;
			}
...
int main(int argc, char *argv[])
{
	...
	if (data.it_count > 100) {
		printf("Number of iterations reduced to 100, due to perfomance reason !\n");
		data.it_count = 100;
	}
