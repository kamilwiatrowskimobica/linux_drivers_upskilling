...
#define MOB_MEMORY_BASE_ADDR	(0x880000000UL) //native Linux
// #define MOB_MEMORY_BASE_ADDR	(0xfec00400UL) //QEMU

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Lukasz Krakowiak");
MODULE_DESCRIPTION("A simple Linux driver with read and write to memory");
MODULE_VERSION("0.1");
...
void exit_handler(uint type_c)
{
	...
	if (type_c & CLEANUP_MAPPED_MEM) {
		iounmap((void __iomem *)io_mem);
		release_mem_region(MOB_MEMORY_BASE_ADDR, MOB_MESSAGE_LEN);
		printk(KERN_INFO "release_mem_region from addr 0x%lx [%d Bytes]\n",
			MOB_MEMORY_BASE_ADDR, MOB_MESSAGE_LEN);
	}
}
			   ...
			   loff_t *offset)
{
	...
	void __iomem *ram_addr = io_mem;
	char tmp[MOB_MESSAGE_LEN];
	char *ptr = tmp;
	...
	result = copy_to_user(buffer, tmp, size);
	if (result != 0) {
			    ...
			    loff_t *offset)
{
	...
	result = copy_from_user(tmp, buffer, len);
	if (result != 0) {
		mutex_unlock(&mdev->lock);
		printk(KERN_ALERT
...
static int __init mobdev_init(void)
{
	...
	// Take over some RAM memory (verify /proc/iomem)
	if (!request_mem_region(MOB_MEMORY_BASE_ADDR, MOB_MESSAGE_LEN, MOB_DEVICE_NAME)) {
		pr_warn("MOBChar: couldn't take over memory from addr 0x%lx\n", MOB_MEMORY_BASE_ADDR);
		return -ENODEV;
	}

	io_mem = ioremap(MOB_MEMORY_BASE_ADDR, MOB_MESSAGE_LEN);
	if (!io_mem) {
		pr_warn("MOBChar: failed ioremap address 0x%lx\n", MOB_MEMORY_BASE_ADDR);
		exit_handler(CLEANUP_MAPPED_MEM);
		return -ENODEV;
	}
	pr_info("MOBChar: io_mem = 0x%p (0x%lx)\n", io_mem, MOB_MEMORY_BASE_ADDR);
	...
	if (IS_ERR(mob_class)) {
		...
		exit_handler(CLEANUP_MAPPED_MEM | CLEANUP_U_REGION);
	...
	if (IS_ERR(mob_devices)) {
		...
		exit_handler(CLEANUP_MAPPED_MEM | CLEANUP_U_REGION | CLEANUP_CLASS_DESTROY);
