...
#define NPAGES		16
#define MMAP_DEV	"/dev/" MOBI_MODULE_NAME
...
static unsigned long show_mem_usage(void)
{
	int fd;
	char buf[40];
	unsigned long mem_usage, ret;
...
int main(int argc, const char **argv)
{
...
#if 0
	// remove nod from prev execution
	system("rm " MMAP_DEV);
#endif
	...
	// if (test >= 2 && test_write_read(fd, addr)) {
	...
	if (test >= 3) {
		...
		if (usage_after_mmap < 0)
			printf("failed to show memory usage\n");
		// printf("after :%lu, before %lu MB\n",
		//        usage_after_mmap >> 20, usage_before_mmap >> 20);
		printf("mmaped :%lu MB\n",
