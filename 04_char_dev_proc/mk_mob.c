 ...
 */
static void *mk_mob_seq_start(struct seq_file *s, loff_t *pos)
{
	if (*pos >= nr_devs)
		return NULL;   /* No more to read */
	return mk_mob_devices + *pos;
}

static void *mk_mob_seq_next(struct seq_file *s, void *v, loff_t *pos)
{
	(*pos)++;
	if (*pos >= nr_devs)
		return NULL;
...
static void mk_mob_seq_stop(struct seq_file *s, void *v)
{
	/* Actually, there's nothing to do here */
}

static int mk_mob_seq_show(struct seq_file *s, void *v)
{
	struct mk_mob_dev *dev = (struct mk_mob_dev *) v;

	seq_printf(s, "\nDevice %i: size %lu, data: %.*s\n",
			(int) (dev - mk_mob_devices), dev->size, (int)dev->size, dev->data);
 ...
 */
static struct seq_operations mk_mob_seq_ops = {
	.start = mk_mob_seq_start,
	.next  = mk_mob_seq_next,
	.stop  = mk_mob_seq_stop,
	.show  = mk_mob_seq_show
};
 ...
 */
static int mk_mob_proc_open(struct inode *inode, struct file *file)
{
	return seq_open(file, &mk_mob_seq_ops);
}

/*
 * Create a set of file operations for our proc file.
 */
static struct proc_ops mk_mob_proc_ops = {
	.proc_open    = mk_mob_proc_open,
	.proc_read    = seq_read,
...
static void mk_mob_create_proc(void)
{
	struct proc_dir_entry *entry;
	entry = proc_create("mk_mob_seq", 0, NULL, &mk_mob_proc_ops);
}

static void mk_mob_remove_proc(void)
{
	/* no problem if it was not registered */
	remove_proc_entry("mk_mob_seq", NULL);
}
