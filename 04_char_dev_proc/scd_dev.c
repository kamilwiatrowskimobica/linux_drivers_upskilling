 ...
 * @date   21 Jule 2023
 * @version 1.0
 * @brief   Character driver with proc for chapter 4 of Linux Device Driver book
 */
...
MODULE_DESCRIPTION("Character driver with proc for chapter 4 of Linux Device Driver book");
MODULE_VERSION("1.0");
...
static int scd_dev_seq_show(struct seq_file *s, void *v)
{
    ...
    seq_printf(s, "scd_dev: %s_%u, size: %zu B, data[%zu B]: %s\n", DEVICE_NAME,
               mdev->id, mdev->buffer_size, mdev->pos, mdev->buffer);
