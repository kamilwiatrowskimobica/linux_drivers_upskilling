...
/* seq declarations*/
static int pwmob_seq_open(struct inode *, struct file *);
static int pwmobdev_seq_show(struct seq_file *, void *);
static void pwmobdev_seq_stop(struct seq_file *, void *);
static void *pwmobdev_seq_start(struct seq_file *, loff_t *);
static void *pwmobdev_seq_next(struct seq_file *, void *, loff_t *);
...
static struct proc_ops seq_pops = { .proc_open = pwmob_seq_open,
				    .proc_read = seq_read,
				    .proc_lseek = seq_lseek,
				    .proc_release = seq_release };

static struct seq_operations seq_ops = { .start = pwmobdev_seq_start,
					 .next = pwmobdev_seq_next,
					 .stop = pwmobdev_seq_stop,
					 .show = pwmobdev_seq_show };
...
static int __init pwmob_dev_init(void)
{
	...
	entry = proc_create("pwmob", 0, NULL, &seq_pops);
	if (entry == NULL)
		return -ENOMEM;
...
static void *pwmobdev_seq_start(struct seq_file *s, loff_t *pos)
{
	if (*pos >= dev_count || *pos < 0)
		return NULL; /* No more devs in array */
	return pwmobCharDevices + *pos;
}
 ...
 *
 */
static void *pwmobdev_seq_next(struct seq_file *s, void *v, loff_t *pos)
{
	(*pos)++;
	if (*pos >= dev_count)
		return NULL;
...
static void pwmobdev_seq_stop(struct seq_file *s, void *v)
{
 ...
 *
 */
static int pwmobdev_seq_show(struct seq_file *s, void *v)
{
	struct pwmob_dev *pwmdev = (struct pwmob_dev *)v;
	if (!pwmdev) {
		pr_warn("failed, no device data\n");
		return -EFAULT;
	}

	seq_printf(s, "Mobdev_%i, data_size = %zu\n",
		   (int)(pwmdev - pwmobCharDevices), pwmdev->data_size);
	return 0;
}

static int pwmob_seq_open(struct inode *inode, struct file *file)
{
