...
static struct seq_operations woab_seq_ops = {
	.start = woab_seq_start,
	.next  = woab_seq_next,
	.stop  = woab_seq_stop,
	.show  = woab_seq_show
};

static const struct proc_ops woab_seq_file_ops = {
        .proc_open    = woab_seq_file_open,
        .proc_read    = seq_read,
...
static int woab_seq_show(struct seq_file *s, void *v)
{
	...
	seq_printf(s, "Driver prints %llu message\n", *spos);
