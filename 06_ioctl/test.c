...
int test_ioctl_reset()
{
    ...
    // Reset device
    if (ioctl(fd, SCD_DEV_IOCTL_RESET) < 0)
    {
        close(fd);
        printf("SCD_DEV_IOCTL_RESET failed");
        return TEST_FAIL;
    }
...
int test_ioctl_write_read()
{
    char *input_buf = "XXXX12345XXXX";
    char buff[BUFFER_LENGTH] = {0};
    scd_dev_ioctl_data_t idata;

    int fd = open(get_device(SCD_0), O_RDONLY);
    if (fd < 0)
    {
        printf("Failed to open the device for write\n");
        return TEST_FAIL;
    ...
    // Write to device
    idata.data = input_buf;
    idata.off = 4;
    idata.size =  strlen(input_buf);
    if (ioctl(fd, SCD_DEV_IOCTL_WRITE, &idata) < 0)
    {
        close(fd);
        printf("SCD_DEV_IOCTL_WRITE failed.\n");
        return TEST_FAIL;
    }
    ...
    idata.off = 8;
    idata.size =  5;
    if (ioctl(fd, SCD_DEV_IOCTL_READ, &idata) < 0)
    {
        close(fd);
        printf("SCD_DEV_IOCTL_READ failed.\n");
        return TEST_FAIL;
    }

    if (memcmp(buff, "12345", 5) != 0)
    {
        printf("Invalid data read from device: %s\n", buff);
        close(fd);
        return TEST_FAIL;
...
int main()
{
    ...
    run_test("ioctl: reset", test_ioctl_reset);
    run_test("ioctl: write/read", test_ioctl_write_read);
