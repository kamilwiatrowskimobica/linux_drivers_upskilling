...
static long scd_dev_ioctl(struct file *filep, unsigned int cmd, unsigned long arg);
...
static long scd_dev_ioctl(struct file *filep, unsigned int cmd, unsigned long arg)
{
    long ret = 0;
    scd_dev_t *mdev = (scd_dev_t *)filep->private_data;
    scd_dev_ioctl_data_t idata;
    void __user *up = (void __user *)arg;

    if (_IOC_TYPE(cmd) != SCD_DEV_IOCTL_MAGIC)
        return -ENOTTY;
    ...
    if (_IOC_DIR(cmd) & (_IOC_READ | _IOC_WRITE))
    {
        ...
        if (_IOC_SIZE(cmd) > sizeof(idata))
            return -ENOTTY;

        if (copy_from_user(&idata, up, _IOC_SIZE(cmd)))
        {
    ...
    if (!mdev)
    {
        printk(KERN_INFO "failed, no device data\n");
        return -EINVAL;
    }
    ...
    switch (cmd)
    {
    case SCD_DEV_IOCTL_RESET:
        memset(mdev->buffer, 0, mdev->buffer_size);
        mdev->pos = 0;
        break;

    case SCD_DEV_IOCTL_WRITE:
        printk(KERN_INFO "SCD_DEV_IOCTL_WRITE!\n");
        if (idata.size + idata.off > mdev->buffer_size)
        {
            ret = -EFBIG;
        }
        else if (copy_from_user(mdev->buffer + idata.off, idata.data,
                                idata.size))
        {
            printk(KERN_INFO "copy from user failed!\n");
            ret = -EFAULT;
        }
        else if (idata.size + idata.off > mdev->pos)
        {
            mdev->pos = idata.size + idata.off;
        }
        break;

    case SCD_DEV_IOCTL_READ:
        printk(KERN_INFO "SCD_DEV_IOCTL_READ!\n");
        if (idata.size + idata.off > mdev->pos)
        {
            ret = -EFBIG;
        }
        else if (copy_to_user(idata.data, mdev->buffer + idata.off,
                              idata.size))
        {
