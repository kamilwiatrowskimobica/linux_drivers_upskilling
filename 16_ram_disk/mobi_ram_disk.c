...
MODULE_DESCRIPTION("Simple Mobi RAM Disk");
MODULE_LICENSE("GPL");
...
static void my_xfer_request(struct my_block_dev *dev, struct request *req)
{
	...
	rq_for_each_segment(bvec, req, iter) {
		...
		pr_alert("%s: buf %8p offset %lu len %u dir %d\n", __func__, buffer, offset, len, dir);
				     ...
				     const struct blk_mq_queue_data *bd)
{
	...
	pr_alert("request received: pos=%llu bytes=%u "
		"cur_bytes=%u dir=%c\n",
...
static int create_block_device(struct my_block_dev *dev)
{
	...
	dev->gd->major = MOBI_BLOCK_MAJOR;
	dev->gd->first_minor = 0;
	dev->gd->fops = &my_block_ops;
	dev->gd->queue = dev->queue;
	dev->gd->private_data = dev;
	snprintf(dev->gd->disk_name, DISK_NAME_LEN, MOBI_DISK_NAME);
...
static int __init my_block_init(void)
{
	...
	err = register_blkdev(MOBI_BLOCK_MAJOR, MOBI_BLKDEV_NAME);
	if (err < 0) {
...
out:
	...
	unregister_blkdev(MOBI_BLOCK_MAJOR, MOBI_BLKDEV_NAME);
	return err;
}
