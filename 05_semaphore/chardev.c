...
#define DEVICE_NAME                                                            \
	"chardev" ///< The device will appear at /dev/ebbchar using this value
#define CLASS_NAME                                                             \
	"chardevclass" ///< The device class -- this is a character device driver
...
static int __init char_init(void)
{
	...
	if (IS_ERR(seq_char_dev_class)) {
		...
		result = PTR_ERR(seq_char_dev_class);
		goto clean_device;
	}
...
cleanup:
	for (i = 0; i < device_count; i++) {
		...
		mutex_destroy(&char_devices[i].lock);
	}
