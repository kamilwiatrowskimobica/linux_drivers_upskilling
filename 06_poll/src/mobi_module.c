...
static unsigned int mobdev_poll(struct file *, struct poll_table_struct *);
			 ...
			 unsigned long arg)
{
	...
	case MOBI_MSG_SET: {
		...
		wake_up_interruptible(&mdev->outq);

		pr_info_ratelimited(
			"MOBI_MSG_SET: %s_%d Received %zu characters from the user\n",
	...
	case MOBI_MSG_GET: {
		...
		pr_info_ratelimited(
			"MOBI_MSG_GET: %s_%d Sent %ld characters to user\n",
...
static unsigned int mobdev_poll(struct file *filep, struct poll_table_struct *wait)
{
	unsigned int mask = 0;
	...
	pr_info_ratelimited("MOBChar: %s, msg_size:%ld\n", __FUNCTION__, mdev->msg_size);

	mutex_lock(&mdev->lock);

	poll_wait(filep, &mdev->outq, wait);

	if (mdev->msg_size > 0) {
		// pr_info_ratelimited(
		// 	"MOBChar: %s_%d msg available to read [m_size:%ld]\n",
		// 	MOB_DEVICE_NAME, MINOR(mdev->cdev.dev), mdev->msg_size);
		mask |= POLLIN | POLLRDNORM; // msg ready
	}
