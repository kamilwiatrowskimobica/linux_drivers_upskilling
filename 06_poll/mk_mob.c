...
static unsigned int mk_mob_dev_poll(struct file *filp, poll_table *wait);
...
static int __init mk_mob_init(void)
{
	...
	for (i = 0; i < nr_devs; i++) {
		...
		init_waitqueue_head(&mk_mob_devices[i].pollq);
	}
...
static long mk_mob_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case MK_MOB_IOC_ENABLE_WRITE:
		dev->wr_enable = true;
	
		/* Wake up the poll, writing possible */
		wake_up_interruptible(&dev->pollq);
		break;

	case MK_MOB_IOC_DISABLE_WRITE:

		/* Wake up the poll, writing not possible */
		wake_up_interruptible(&dev->pollq);
		dev->wr_enable = false;
		break;
...
static unsigned int mk_mob_dev_poll(struct file *filp, poll_table *wait)
{
	...
	pr_debug_ratelimited("Poll enter\n");

	if (down_interruptible(&dev->sem))
		return -ERESTARTSYS;
	
	poll_wait(filp, &dev->pollq, wait);
