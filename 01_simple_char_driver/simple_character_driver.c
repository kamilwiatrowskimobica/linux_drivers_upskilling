/**
 * @file   simple_character_driver.c
 * @author Kamil Jamros
...
#define DEVICE_NAME                                                            \
	"mob" ///< The device will appear at /dev/Mob using this value
#define CLASS_NAME                                                             \
	"mob" ///< The device class -- this is a character device driver

MODULE_LICENSE(
	"GPL"); ///< The license type -- this affects available functionality
MODULE_AUTHOR("Kamil Jamros"); ///< The author -- visible when you use modinfo
MODULE_DESCRIPTION(
	"A simple Linux char driver"); ///< The description -- see modinfo
...
static struct class *MobClass =
	NULL; ///< The device-driver class struct pointer
static struct device *MobDevice =
...
{
	printk(KERN_INFO "Mob: Initializing the Mob LKM\n");
	...
	if (IS_ERR(MobClass)) { // Check for error and clean up if there is
		unregister_chrdev(majorNumber, DEVICE_NAME);
		printk(KERN_ALERT "Failed to register device class\n");
		return PTR_ERR(
			MobClass); // Correct way to return an error on a pointer
	}
	...
	MobDevice = device_create(MobClass, NULL, MKDEV(majorNumber, 0), NULL,
				  DEVICE_NAME);
	if (IS_ERR(MobDevice)) { // Clean up if there is an error
		class_destroy(
			MobClass); // Repeated code but the alternative is goto statements
		unregister_chrdev(majorNumber, DEVICE_NAME);
		printk(KERN_ALERT "Failed to create the device\n");
		return PTR_ERR(MobDevice);
	}
	printk(KERN_INFO
	       "Mob: device class created correctly\n"); // Made it! device was initialized
	return 0;
}
			...
			loff_t *offset)
{
	...
	if (error_count == 0) { // if true then have success
		...
		printk(KERN_INFO
		       "Mob: Failed to send %d characters to the user\n",
		       error_count);
			 ...
			 loff_t *offset)
{
	...
	size_of_message =
		strlen(message); // store the length of the stored message
	printk(KERN_INFO "Mob: Received %zu characters from the user\n", len);
	return len;
}
