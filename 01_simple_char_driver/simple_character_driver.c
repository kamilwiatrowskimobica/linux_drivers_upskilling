...
#define  DEVICE_NAME "mob"        ///< The device will appear at /dev/Mob using this value
...
static struct class*  MobClass  = NULL; ///< The device-driver class struct pointer
static struct device* MobDevice = NULL; ///< The device-driver device struct pointer
...
static int __init Mob_init(void){
   ...
   if (IS_ERR(MobClass)){                // Check for error and clean up if there is
      ...
      return PTR_ERR(MobClass);          // Correct way to return an error on a pointer
   }
   printk(KERN_INFO "Mob: device class registered correctly\n");

   // Register the device driver
   MobDevice = device_create(MobClass, NULL, MKDEV(majorNumber, 0), NULL, DEVICE_NAME);
   if (IS_ERR(MobDevice)){               // Clean up if there is an error
      class_destroy(MobClass);           // Repeated code but the alternative is goto statements
      unregister_chrdev(majorNumber, DEVICE_NAME);
