...
#define DEVICE_NAME                                                            \
	"mobchar" ///< The device will appear at /dev/ using this value
#define CLASS_NAME                                                             \
...
static struct class *mobcharClass =
	NULL; ///< The device-driver class struct pointer
static struct device *mobcharDevice =
...
static int __init char_write_read_init(void)
{
	...
	if (deviceNumber < 0) {
		printk(KERN_ALERT
		       "%s failed to register a device number for %s\n",
	...
	mobcharDevice = device_create(
		mobcharClass, NULL, MKDEV(deviceNumber, 0), NULL, DEVICE_NAME);
	// Check for error and clean up if there is
...
static void __exit char_write_read_exit(void)
{
	...
	device_destroy(mobcharClass,
		       MKDEV(deviceNumber, 0)); // remove the device
	class_destroy(mobcharClass); // remove the device class
	unregister_chrdev(deviceNumber,
...
static int dev_open(struct inode *inodep, struct file *filep)
{
	if (!mutex_trylock(&mobchar_mutex)) {
		printk(KERN_ALERT "%s Device in use by another process",
		       PRINT_PREFIX);
		return -EBUSY;
	}
	opensCounts++;
			 ...
			 loff_t *offset)
{
	...
	if (error_count == 0) {
		...
		printk(KERN_INFO
		       "%s Failed to receive characters from the user\n",
		       PRINT_PREFIX);
		return -EFAULT;
	}
	return len;
