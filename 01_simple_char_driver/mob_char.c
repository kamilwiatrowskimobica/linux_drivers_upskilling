...
#define DEVICE_NAME "mob" // The device will appear as /dev/mobchar
#define CLASS_NAME "mobica" // The device class name - /sys/class/mobica/
#define MSG_MAX_SIZE 256

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Lukasz Sokolowski");
			...
			loff_t *offset)
{
	...
	if (len > MSG_MAX_SIZE || len < data_size) {
		printk(KERN_WARNING
		       "Mob: wrong buffer size. Aborting read\n");
		return -EFBIG;
	}
			 ...
			 loff_t *offset)
{
	...
	if (len > MSG_MAX_SIZE) {
		printk(KERN_WARNING
		       "Mob: wrong buffer size %zu. Aborting write\n",
		       len);
		return -EFBIG;
	}
	...
	if (error_count != 0) {
		printk(KERN_INFO
		       "Mob: failed to read %d characters from user\n",
		       error_count);
		return -EFAULT;
	}
...
static int __init mobchar_init(void)
{
	...
	if (major_number < 0) {
		printk(KERN_ALERT
		       "Mob: failed to register a major number\n");
		return major_number;
	}
	printk(KERN_INFO "Mob: registered with major number %d\n",
	       major_number);
