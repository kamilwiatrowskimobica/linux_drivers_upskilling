 ...
 * http://www.derekmolloy.ie/ linux device drivers course.
 */
...
#define  DEVICE_NAME "simplechar"    ///< The device will appear at /dev/SimpleChar using this value
...
static struct class*  simplecharClass  = NULL; ///< The device-driver class struct pointer
static struct device* simplecharDevice = NULL; ///< The device-driver device struct pointer
...
static int __init simplechar_init(void){
   ...
   if (IS_ERR(simplecharClass)){                // Check for error and clean up if there is
      unregister_chrdev(majorNumber, DEVICE_NAME);
      printk(KERN_ALERT "Failed to register device class\n");
      return PTR_ERR(simplecharClass);          // Correct way to return an error on a pointer
   }
   ...
   if (IS_ERR(simplecharDevice)){               // Clean up if there is an error
      class_destroy(simplecharClass);           // Repeated code but the alternative is goto statements
      unregister_chrdev(majorNumber, DEVICE_NAME);
...
static void __exit simplechar_exit(void){
   device_destroy(simplecharClass, MKDEV(majorNumber, 0));     // remove the device
   class_destroy(simplecharClass);                             // remove the device class
...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset){
   ...
   if (error_count != 0)
   {
       printk(KERN_INFO "SimpleChar: Failed to receive %d characters to the user\n", error_count);
       return -EFAULT;              // Failed -- return a bad address message (i.e. -14)
   }
