...
static int __init mobchar_init(void) {
  printk(KERN_INFO "mobChar: Initializing the mobChar LKM\n");
  ...
  if (IS_ERR(mobcharClass)) { // Check for error and clean up if there is
    unregister_chrdev(majorNumber, DEVICE_NAME);
    printk(KERN_ALERT "Failed to register device class\n");
    return PTR_ERR(mobcharClass); // Correct way to return an error on a pointer
  }
  printk(KERN_INFO "mobChar: device class registered correctly\n");

  // Register the device driver
  mobcharDevice = device_create(mobcharClass, NULL, MKDEV(majorNumber, 0), NULL,
                                DEVICE_NAME);
  if (IS_ERR(mobcharDevice)) { // Clean up if there is an error
    class_destroy(
        mobcharClass); // Repeated code but the alternative is goto statements
    unregister_chrdev(majorNumber, DEVICE_NAME);
    printk(KERN_ALERT "Failed to create the device\n");
    return PTR_ERR(mobcharDevice);
  }
  printk(KERN_INFO
         "mobChar: device class created correctly\n"); // Made it! device was
                                                       // initialized
  return 0;
}
...
static void __exit mobchar_exit(void) {
  device_destroy(mobcharClass, MKDEV(majorNumber, 0)); // remove the device
  class_unregister(mobcharClass);              // unregister the device class
