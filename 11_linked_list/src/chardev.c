...
static void woab_device_cleanup(void)
{
	...
	for (i = 0; i < woabchar_nr_devices; i++) {
		...
		woab_list_clear(&woab_device[i].list);
	}
	kfree(woab_device);
}
...
static int __init woab_devchar_init(void)
{
	...
	for (i = 0; i < woabchar_nr_devices; i++) {
		...
		woab_list_init(&woab_device[i].list);
				 ...
				 loff_t *offset)
{
	...
	woab_list_print(&wdev->list);

	if ((ret = woab_irq_trigger(wdev->index))) {
		return ret;
	}
